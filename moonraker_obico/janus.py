...
class JanusConn:
    ...
    def start(self):
        ...
        if os.getenv("JANUS_SERVER", "").strip() != "":
            _logger.warning(
                "Using an external Janus gateway. Not starting the built-in Janus gateway."
            )
            self.start_janus_ws()
            return
        ...
        def run_janus_forever():
            ...
            def setup_janus_config():
                video_enabled = (
                    "true"
                    if pi_version()
                    and self.config.webcam.disable_video_streaming is not True
                    else "false"
                )
                ...
                setup_proc = subprocess.Popen(
                    setup_cmd.split(), stdout=subprocess.PIPE, stderr=subprocess.STDOUT
                )

                returncode = setup_proc.wait()
                (stdoutdata, stderrdata) = setup_proc.communicate()
                if returncode != 0:
                    raise JanusNotSupportedException(
                        "Janus setup failed. Skipping Janus connection. Error: \n{}".format(
                            stdoutdata
                        )
                    )
            ...
            def run_janus():
                ...
                self.janus_proc = subprocess.Popen(
                    janus_cmd.split(), stdout=subprocess.PIPE, stderr=subprocess.STDOUT
                )

                while not self.shutting_down:
                    line = to_unicode(
                        self.janus_proc.stdout.readline(), errors="replace"
                    )
                    if line:
                        ...
                        raise Exception(
                            "Janus quit! This should not happen. Exit code: {}".format(
                                self.janus_proc.returncode
                            )
                        )
        ...
        self.use_camera_streamer_rtsp = self.app_model.linked_printer.get(
            "is_pro"
        ) and is_port_open("127.0.0.1", CAMERA_STREAMER_RTSP_PORT)
        _logger.debug(f"Using camera streamer RSTP? {self.use_camera_streamer_rtsp}")

        self.webcam_streamer = WebcamStreamer(
            self.app_model, self.server_conn, self.sentry, self
        )
